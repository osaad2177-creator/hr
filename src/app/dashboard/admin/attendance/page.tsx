...
import { AttendanceRecord } from '@/lib/types/index';
...
export default function AttendancePage() {
  ...
  const load = async (df = dateFilter, sf = statusFilter) => {
    setLoading(true);
    const data = await getAttendanceRecords({
      startDate: df || undefined,
      endDate: df || undefined,
      status: (sf as AttendanceRecord['status']) || undefined,
    });
    setRecords(data);
    setLoading(false);
  };

  useEffect(() => { load(dateFilter, statusFilter); }, [dateFilter, statusFilter]); // eslint-disable-line react-hooks/exhaustive-deps
